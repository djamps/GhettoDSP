...
void setSysGain() {
  ...
  switch ( settings.channelMode ) {
    ...
    case 1: // Left
      ...
      dsp.volume(MOD_STEREOCOMBINE_ALG0_STAGE0_VOLTWO_ADDR, -30);
      break;
    case 2: // Right
      dsp.volume(MOD_STEREOCOMBINE_ALG0_STAGE0_VOLONE_ADDR, -30);
...
void startAudio() {
  ...
  //pinMode(DSP_SELFBOOT, INPUT); // don't need selfboot
  ...
  // Wait for DSP to finish self boot before doing anything
  delay(1000);

  // Tell PWR to turn on amp
#if PWR
  audioDetected = 1;
  sendDataToPWR();
#endif
...
void checkForAudio() {
  if ( dsp.readBack(MOD_READBACK1_ALG0_VAL0_ADDR, MOD_READBACK1_ALG0_VAL0_VALUES, 3) == 0 ) {
    //if ( audioDetected != 1 && millis() > 5000 ) {
    if ( audioDetected != 1 ) {
      if ( DEBUG ) {
